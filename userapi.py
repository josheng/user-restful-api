...
parser.add_argument('name', type=str, help='Name of the user cannot be blank!', required=True)
parser.add_argument('dob', type=lambda x: datetime.strptime(x,"%d/%m/%Y").date().strftime("%d/%m/%Y"), help='Date of Birth of the User must be in DD/MM/YYYY format, e.g 30/01/2022', required=True)
parser.add_argument('address', type=str, help='Address of the user cannot be blank!', required=True)
parser.add_argument('description', type=str, help='Description of the User')
...
class UserAPI(Resource):
    ...
    def post(self):
        ...
        last_user = db.user_db.find().limit(1).sort([( '$natural', -1 )] )
        return loads(dumps(last_user))
